...
| [bankr](https://bankr.bot) | [token-strategist](token-strategist/) | Token launch strategist. Evaluate token concepts against five market forces, then deploy on Base via Bankr CLI with built-in fee splits that fund the agent. |
...
│       └── bankr.sh
│
├── token-strategist/  # Token launch strategy + deploy
